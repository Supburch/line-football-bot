...
def create_app():
    ...
    def start_timer():
        ...
        if not Config.BASE_URL and request.host_url:
            ...
            if (
                host_url.startswith("http://")
                and "localhost" not in host_url
                and "127.0.0.1" not in host_url
            ):
    ...
    def handle_exception(e):
        logger.error(
            {"event": "unhandled_exception", "error": str(e), "traceback": traceback.format_exc()}
        )
