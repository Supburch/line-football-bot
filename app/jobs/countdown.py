...
from app.utils.constants import WORLD_CUP_START, TERMINAL_MATCH_STATUSES
...
from app.utils.free_tv_schedule import format_free_tv_section
        ...
        if delta > 0:
            ...
            if tournament_active:
                ...
                if isinstance(all_data, dict):
                    ...
                    final_match = next((m for m in all_matches if m.get("stage") == "FINAL"), None)
                    if final_match:
                        tournament_active = final_match.get("status") not in TERMINAL_MATCH_STATUSES
                    elif all_matches:
                        tournament_active = any(
                            m.get("status") not in TERMINAL_MATCH_STATUSES for m in all_matches
                        )
            ...
            greeting_text = f"🌅 สวัสดีตอนเช้าครับแฟนบอลโลก! 🏆\n\n" f'💬 "{quote}"'
