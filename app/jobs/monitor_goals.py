...
from app.flex.flex_builders import build_var_flex, build_penalty_shootout_flex, build_red_card_flex
from app.utils.constants import (
    EPL_CODE,
    ACTIVE_COMPETITION,
    CLEANUP_MATCH_STATUSES,
    is_live_score_enabled,
)
...
def _monitor_goals_inner(live_matches: list = None):
    try:
        ...
        if not is_live_score_enabled():
            logger.info(
                "live_score_disabled_wc",
                extra={"reason": "LIVE_SCORE_WC_DISABLED=True during WC 26"},
            )
            return
        ...
        for m in matches:
            ...
            if utc_str:
                try:
                    ...
                    if diff_seconds > 12600:
                        logger.warning(
                            "stale_match_ignored_by_time",
                            extra={"match_id": fid, "diff_hrs": diff_seconds / 3600},
                        )
                    ...
                    if prev_score and (final_hs, final_as) != prev_score:
                        ...
                        if new_total > old_total:
                            ...
                            if not get_sent_event(event_key):
                                logger.info(
                                    "final_whistle_goal_silent",
                                    extra={
                                        "match_id": fid,
                                        "score": f"{final_hs}-{final_as}",
                                        "prev": f"{prev_score[0]}-{prev_score[1]}",
                                    },
                                )
            ...
            if is_pso:
                ...
                if prev_pen is not None:
                    ...
                    if pen_hs > prev_pen_hs or pen_as > prev_pen_as:
                        ...
                        if state_manager.can_retry_event(event_key_pen) and not get_sent_event(
                            event_key_pen
                        ):
                            if pen_hs > prev_pen_hs:
                                scorer_text = (
                                    f"🎯 {home_name} SCORED penalty! ({pen_hs} - {pen_as})"
                                )
                            else:
                                scorer_text = (
                                    f"🎯 {away_name} SCORED penalty! ({pen_hs} - {pen_as})"
                                )
                            ...
                            if result in {BroadcastResult.SUCCESS, BroadcastResult.PARTIAL}:
                                commit_res = commit_match_state(
                                    pen_fid, event_key_pen, pen_hs, pen_as
                                )
                                if commit_res.success:
                                    ...
                                    logger.info(
                                        "pso_score_committed",
                                        extra={"match_id": pen_fid, "event_key": event_key_pen},
                                    )
                                else:
                                    abandoned = state_manager.register_event_failure(
                                        event_key_pen, is_fatal=False
                                    )
                                    if abandoned:
                                        state_manager.commit_memory(pen_fid, pen_hs, pen_as)
            ...
            if state_manager.get_score(fid) is None:
                ...
                if get_sent_event(event_key):
                    ...
                    if db_score and db_score == (hs, as_):
                        state_manager.commit_memory(fid, hs, as_)
                        logger.info(
                            "recovery_from_db_score",
                            extra={"match_id": fid, "score": f"{hs}-{as_}"},
                        )
                        continue
                    ...
                    state_manager.commit_memory(fid, hs, as_)
                    logger.info(
                        "safe_init_current_score", extra={"match_id": fid, "score": f"{hs}-{as_}"}
                    )
                    continue

            # Detect Transition
            is_goal, is_var, goal_diff, prev_score, score_shifted = detect_score_transition(
                fid, hs, as_
            )
            ...
            if not is_goal and not is_var:
                if score_shifted:
                    ...
                    state_manager.commit_memory(fid, hs, as_)
                    logger.info(
                        "score_shifted_silently", extra={"match_id": fid, "score": f"{hs}-{as_}"}
                    )
            ...
            if is_var:
                if should_ignore_rollback(goal_diff, status, fid):
                    ...
                    logger.warning(
                        "suspicious_rollback_memory_retained",
                        extra={
                            "match_id": fid,
                            "kept_score": f"{prev_score[0]}-{prev_score[1]}",
                            "ignored_score": f"{hs}-{as_}",
                        },
                    )
                    continue
            ...
            if is_var:
                ...
                flex_msg = build_var_flex(
                    home_name, away_name, hs, as_, h_logo, a_logo, scorer, comp_code=comp_code
                )
                logger.info(
                    "var_detected",
                    extra={
                ...
                _process_red_cards_for_match(
                    m, fid, home_name, away_name, hs, as_, h_logo, a_logo, comp_code
                )
                continue
            ...
            if result in {BroadcastResult.SUCCESS, BroadcastResult.PARTIAL}:
                ...
                if commit_res.success:
                    ...
                    logger.warning(
                        "db_commit_failed_after_broadcast",
                        extra={"match_id": fid, "event_key": event_key},
                    )
...
def _process_red_cards_for_match(
    m: dict,
    fid: str,
    home_name: str,
    away_name: str,
    ...
    try:
        ...
        for booking in bookings:
            ...
            if result in {BroadcastResult.SUCCESS, BroadcastResult.PARTIAL}:
                ...
                logger.warning(
                    "red_card_broadcast_failed",
                    extra={"event_key": rc_event_key, "result": str(result)},
                )
    except Exception as e:
