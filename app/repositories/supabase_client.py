    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt == max_retries - 1:
                logger.error(
                    {"event": "supabase_query_failed", "error": str(e), "attempts": max_retries}
                )
                return None
            time.sleep(1)
...
def db_get_groups() -> List[str]:
    ...
    res = execute_with_retry(
        supabase.table("football_groups").select("group_id").eq("active", True)
    )
    if res and res.data:
...
def commit_match_state(
    match_id: str, event_key: str, home_score: int, away_score: int
) -> CommitResult:
    ...
    if not event_saved:
        logger.error(
            {
                "event": "split_brain_warning",
                "message": "Failed to save event_key",
                "event_key": event_key,
            }
        )
    ...
    if not score_saved:
        logger.error(
            {
                "event": "split_brain_warning",
                "message": "Failed to save match score after saving event",
                "match_id": match_id,
            }
        )
