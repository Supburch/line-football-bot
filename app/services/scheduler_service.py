...
def start_scheduler():
    ...
    scheduler.add_job(
        lambda: run_smart_schedule(scheduler),
        "cron",
        minute="0,30",
        id="smart_schedule",
        max_instances=1,
    ...
    scheduler.add_job(
        log_state_manager_health,
        "cron",
        minute="0,30",
        id="state_manager_health",
        max_instances=1,
        coalesce=True,
        replace_existing=True,
    )
    ...
    def _async_startup_greeting_check():
        ...
        try:
            ...
            if 8 <= now_bkk.hour < 10:
                logger.info(
                    "startup_checking_missed_morning_greeting", extra={"current_hour": now_bkk.hour}
                )
                check_world_cup_countdown()
                send_dome_fc_morning_greeting()
            else:
                logger.info(
                    "startup_greeting_check_skipped_outside_window",
                    extra={"current_hour": now_bkk.hour},
                )
