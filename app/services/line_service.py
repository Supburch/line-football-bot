...
def broadcast(msg: Union[str, FlexDict, list[Union[str, FlexDict]]]) -> BroadcastResult:
    ...
    for m in raw_msgs:
        if isinstance(m, dict):
            if m.get("type") == "image":
                ...
                line_msgs.append(
                    FlexMessage(alt_text="EPL Alert", contents=FlexContainer.from_dict(m))
                )
        else:
    ...
    def _send(gid: str) -> bool:
        try:
            with ApiClient(line_config) as client:
                MessagingApi(client).push_message(PushMessageRequest(to=gid, messages=line_msgs))
            return True
    ...
    try:
        ...
        logger.error(
            {"event": "broadcast_timeout", "message": "Line push message timed out after 30s"}
        )
